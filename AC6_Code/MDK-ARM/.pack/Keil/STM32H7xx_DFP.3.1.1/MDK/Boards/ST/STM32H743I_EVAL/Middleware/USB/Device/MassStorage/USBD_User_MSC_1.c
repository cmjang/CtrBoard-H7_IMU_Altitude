...
extern 
volatile uint8_t  usbd_msc1_media_own;
volatile uint8_t  usbd_msc1_media_own;  // USB MSC1 media ownership
...
void USBD_MSC1_Initialize (void) {
  ...
  usbd_msc1_media_own = MEDIA_OWN_USB;  // Initially media is owned by USB
...
static uint8_t  media_ready_ex = 0U;    // Previous media ready state
  ...
  if ((own & MEDIA_OWN_CHG)        ||           // If owner change requested or
    ...
    if (own & MEDIA_OWN_CHG) {
      usbd_msc1_media_own &= ~MEDIA_OWN_CHG;    // Clear request to change media owner if it was handled
    }
  ...
  if ((!media_ok) || (!(usbd_msc1_media_own & MEDIA_OWN_USB))) {
    return 0U;
  }
