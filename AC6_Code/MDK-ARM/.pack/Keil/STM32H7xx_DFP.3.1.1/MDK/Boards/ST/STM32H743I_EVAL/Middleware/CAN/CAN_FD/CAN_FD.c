 ...
 *------------------------------------------------------------------------------
 * Name:    CAN_FD.c
 * Purpose: CAN FD Example - CAN Interface module
 *----------------------------------------------------------------------------*/
...
#define  CAN_DATA_LEN          (64)       // Number of data bytes in message
#define  CAN_BITRATE_NOMINAL   (125000)   // Nominal bitrate (125 kbit/s)
#define  CAN_BITRATE_FD_DATA   (1000000)  // FD data phase bitrate (1 Mbit/s)
...
static bool CAN_Initialize (void) {
  ...
  if ((clock % (8U*CAN_BITRATE_FD_DATA)) == 0U) {                               // If CAN base clock is divisible by 8 * nominal bitrate without remainder
    ...
    status = ptrCAN->SetBitrate    (ARM_CAN_BITRATE_FD_DATA,                    // Set FD data phase bitrate 
                                    CAN_BITRATE_FD_DATA,                        // Set FD data phase bitrate to configured constant value
                                    ARM_CAN_BIT_PROP_SEG  (5U) |                // Set propagation segment to 5 time quanta
                                    ARM_CAN_BIT_PHASE_SEG1(1U) |                // Set phase segment 1 to 1 time quantum (sample point at 87.5% of bit time)
                                    ARM_CAN_BIT_PHASE_SEG2(1U) |                // Set phase segment 2 to 1 time quantum (total bit is 8 time quanta long)
                                    ARM_CAN_BIT_SJW       (1U));                // Resynchronization jump width is same as phase segment 2
  } else if ((clock % (10U*CAN_BITRATE_FD_DATA)) == 0U) {                       // If CAN base clock is divisible by 10 * nominal bitrate without remainder
    status = ptrCAN->SetBitrate    (ARM_CAN_BITRATE_NOMINAL,                    // Set nominal bitrate
                                    CAN_BITRATE_NOMINAL,                        // Set nominal bitrate to configured constant value
    ...
    status = ptrCAN->SetBitrate    (ARM_CAN_BITRATE_FD_DATA,                    // Set FD data phase bitrate
                                    ...
                                    ARM_CAN_BIT_PHASE_SEG2(1U) |                // Set phase segment 2 to 1 time quantum (total bit is 10 time quanta long)
                                    ARM_CAN_BIT_SJW       (1U));                // Resynchronization jump width is same as phase segment 2
  } else if ((clock % (12U*CAN_BITRATE_FD_DATA)) == 0U) {                       // If CAN base clock is divisible by 12 * nominal bitrate without remainder
    status = ptrCAN->SetBitrate    (ARM_CAN_BITRATE_NOMINAL,                    // Set nominal bitrate
                                    CAN_BITRATE_NOMINAL,                        // Set nominal bitrate to configured constant value
                                    ARM_CAN_BIT_PROP_SEG  (7U) |                // Set propagation segment to 7 time quanta
  ...
  status = ptrCAN->Control (ARM_CAN_SET_FD_MODE, 1);                            // Activate FD mode
