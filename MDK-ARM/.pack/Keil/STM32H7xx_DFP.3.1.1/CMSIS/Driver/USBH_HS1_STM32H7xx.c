 ...
 * Project:      USB High/Full/Low-Speed Host Driver for ST STM32H7xx
 * -------------------------------------------------------------------------- */

/*! \page stm32h7_usbh0 USB Host 0
...
:-----------------------|:-------------:|:---------:|:-----------
USBH0_MAX_PIPE_NUM      |    **16**     |      16   | Defines maximum number of Pipes that the driver will support. This value impacts driver memory requirements.
USBH0_BULK_IN_NAK_DELAY |     **0**     |   65535   | Delay between NAKs on the Bulk IN pipe (in SOFs), value 0 = not used (no delay between NAKs)

## STM32CubeMX

The USB Host driver requires:
  - **USB peripheral clock** at **48 MHz**
  - **USB_OTG_HS** peripheral configured as **Host_Only**
...
:----------:|:-----------------------------|:-----------
USB_OTG_HS  | Host_Only                    | USB Controller
...
\parblock
\note For the custom VBUS handling, please provide custom implementation of the **USBH0_VbusOnOff** function
\note For the custom over-current detection, please provide custom implementation of the **USBH0_GetOC** function
\endparblock
...
### Pinout & Configuration tab
  ...
  2. Under **Categories**: **Connectivity** select **USB_OTG_HS**:
...
### Project Manager tab
  ...
  1. Under **Advanced Settings**:
     ...
     __Generated Function Calls__:
        ...
        :-------------|:---------------------------:|:----------------:|:-----------------:|:-------------------:
        checked       | MX_USB_OTG_HS_HCD_Init      | USB_OTG_HS       | **checked**       | checked
...
#if    (OTG_HS_HOST_USED == 1)          // If OTG_HS is enabled and configured in Host mode in the CubeMX
...
#ifndef USBH0_USE_DMA
#define USBH_USE_DMA                   (0U)
#else
#error USBH_HS1 Driver: DMA not supported!
#endif

#ifndef USBH0_DATA_CACHED
#define USBH_DATA_CACHED               (0U)
#else
#error USBH_HS1 Driver: Cache handling not supported!
#endif
...
#define OTG_HOST                       ((USB_OTG_HostTypeDef *)       ((uint32_t)USB1_OTG_HS_PERIPH_BASE + USB_OTG_HOST_BASE))
#define OTG_HPRT                      *(__IO uint32_t *)              ((uint32_t)USB1_OTG_HS_PERIPH_BASE + USB_OTG_HOST_PORT_BASE)
#define OTG_PCGCCTL                   *(__IO uint32_t *)              ((uint32_t)USB1_OTG_HS_PERIPH_BASE + USB_OTG_PCGCCTL_BASE)
#define OTG_HC_PTR(ch)                 ((USB_OTG_HostChannelTypeDef *)((uint32_t)USB1_OTG_HS_PERIPH_BASE + USB_OTG_HOST_CHANNEL_BASE + ((ch) * USB_OTG_HOST_CHANNEL_SIZE)))
#define USB_IRQn                       (OTG_HS_IRQn)

#if (USBH_USE_DMA == 0U)                // If DMA is not used (Slave Mode)
static const uint32_t *OTG_DFIFO[] = { (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE                            ),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 1U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 2U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 3U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 4U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 5U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 6U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 7U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 8U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + ( 9U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (10U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (11U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (12U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (13U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (14U * USB_OTG_FIFO_SIZE)),
                                       (uint32_t *)(USB1_OTG_HS_PERIPH_BASE + USB_OTG_FIFO_BASE + (15U * USB_OTG_FIFO_SIZE))
                                     };
#endif
...
extern void USBH0_VbusOnOff (bool vbus);
extern bool USBH0_GetOC     (void);
...
/**
  ...
  \return      none
*/
__WEAK void USBH0_VbusOnOff (bool vbus) { (void)vbus; }
...
*/
__WEAK bool USBH0_GetOC (void) {
#if (OTG_HS_ULPI_USED == 1)
  return ((OTG_HPRT & USB_OTG_HPRT_POCA) != 0U);
#else
  return false;
#endif
}
...
static int32_t USBH_PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    ...
    case ARM_POWER_FULL:
      ...
      __HAL_RCC_USB1_OTG_HS_FORCE_RESET();              // Reset OTG module
      HAL_Delay(1U);
      __HAL_RCC_USB1_OTG_HS_RELEASE_RESET();            // Clear reset of OTG module
      HAL_Delay(1U);

#if  (OTG_HS_ULPI_USED == 0)                            // Internal Full-speed PHY
...
#else                                                   // External ULPI High-speed PHY
...
static int32_t USBH_PortVbusOnOff (uint8_t port, bool vbus) {
  ...
  USBH0_VbusOnOff (vbus);               // User overridable callback
...
static ARM_USBH_PORT_STATE USBH_PortGetState (uint8_t port) {
  ...
  *((volatile uint32_t *)&port_state) = 0U;
  ...
  port_state.overcurrent = USBH0_GetOC ();
...
/**
  \fn          void USBH_HS1_IRQ (uint32_t gintsts)
  \brief       USB Host OTG_HS1 Interrupt Routine (IRQ).
*/
void USBH_HS1_IRQ (uint32_t gintsts) {
  PIPE_t                     *ptr_pipe;
  USB_OTG_HostChannelTypeDef *ptr_ch;
...
#if  (OTG_HS_ULPI_USED == 1)                            // External ULPI High-speed PHY
